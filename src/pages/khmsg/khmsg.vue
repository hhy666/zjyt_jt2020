  ...
  <view class='khmsg'>
    ...
    <view class="khmsg_table">
      <view class="khmsg_title">
        ...
        <a
          :class="{expand:gseflag, unexpand: !gseflag}"
          @click="qhGseflag()"
        ></a>
      </view>
      <view
    ...
    >
      <view class="khmsg_title">
        ...
        <a
          :class="{expand:gdeflag, unexpand: !gdeflag}"
          @click="qhGdeflag()"
        ></a>
      </view>
      <view
        class="khmsg_tr"
        v-for="(item,index) in gdData"
        :key="index"
        v-if="gdeflag"
        :style="{lineHeight: (gdeflag && (item.td0 != '' || item.td1 != '' ) ? 0 : '30Px')}"
      >
        <view
          class="khmsg_td"
          :class="{w100: item.td1 == '', gdc: item.gdname, dgd: item.dgd }"
        >{{item.td0}}</view>
        <view
          class="khmsg_td"
          :class="{cgbl: item.cgbl}"
          v-if="item.td1 != ''"
        >{{item.td1}}</view>
      </view>
      <text
    ...
    >
      <view class="khmsg_title">
        ...
        <a
          :class="{expand:ggeflag, unexpand: !ggeflag}"
          @click="qhGgeflag()"
        ></a>
      </view>
      <view
    ...
    >
      <view class="khmsg_title">
        ...
        <a
          :class="{expand:fzjgeflag, unexpand: !fzjgeflag}"
          @click="qhFzjgeflag()"
        ></a>
      </view>
      <view
        class="khmsg_tr"
        v-for="(item,index) in fzjgData"
        :key="index"
        v-if="fzjgeflag"
      >
        <view
          class="khmsg_td"
          :class="{w100: item.td1 == '', gdc: item.gdname, ggc: item.ggname }"
        >{{item.td0}}</view>
        <view
          class="khmsg_td"
          v-if="item.td1 != ''"
...
export default {
  ...
  created() {
    ...
    // 判断 khmc 是否存在 不存在就获取id 进行查询获取客户名称
    if (this.khmc == null || this.khmc == "" || this.khmc == undefined) {
      this.findKhmcByOrgId();
    } else {
      this.initGsData();
    }
  ...
  methods: {
      ...
      requestData(
        {
          ...
          operId: "findKhmcByOrgId",
          data: { companyId: Taro.getStorageSync("orgId") },
        },
        (response) => {
          if (
            response.data.data.khmc == undefined ||
            response.data.data.khmc == null
          ) {
            _this.khmc = "查无此公司";
          } else {
            _this.khmc = response.data.data.khmc;
          }
    ...
    navigateToback() {
      Taro.redirectTo({
        url: Taro.getStorageSync("href") + "?s=" + Math.random(),
      });
    },
    setTabFixed() {
      const _this = this;
    ...
    initGsData() {
      ...
      requestData(
        ...
        (response) => {
          ...
          if (
            response.data.data.tdata == undefined ||
            response.data.data.tdata.length == 0
          ) {
