<template>
  ...
  >
    <view
      class="sc_outer_kuang"
      :style="{height:pmHeight+'px'}"
    ></view>
    <view
      class="sc_inner_kuang"
      :style="{height:pmHeight*0.78+'px'}"
    >
      ...
      <view class="scik_body">
        <view class="scikb_left">
          <a
            class="scikbl_tr fx100"
...
export default {
    ...
    requestData(
      {
        ...
        operId: "findCompanyMsgForChange",
  ...
  methods: {
    ...
    submit() {
      ...
      Taro.setStorageSync(
        "showType",
        this.childrenData[this.choosedChildIndex].id
      );
      Taro.setStorageSync("showArea", 1);
      Taro.setStorageSync(
        "showTypeName",
        this.childrenData[this.choosedChildIndex].gsname
      );

      Taro.reLaunch({
        url: "/pages/jtgs/jtgs?s=" + Math.random(),
      });
    },
  },
};
